...
if($_POST['go']){
    ...
    $advqry=mysqli_query($config,"SELECT * FROM clinicals WHERE ClinicalMonth='$advancemonth' AND mno='$mno'");
    $prevAdvance=0;
    if(mysqli_num_rows($advqry)>0){
        while($advrow=mysqli_fetch_assoc($advqry)){
            $currentadv=$advrow['Amount'];
...
if($_POST['submit']){
    ...
    if(empty($mno)){
        ...
        if(empty($amount)){
            ...
            if(mysqli_query($config,"INSERT INTO clinicals(InvoiceNo,mno,Names,Amount,Date_Time,ClinicalMonth) VALUES('$invoice','$mno','$names','$amount','$date','$month')")){
                $form='<img src="images/success.png" width="23" height="23" align="left">Clinical recorded successfully.'; 
                $_SESSION['month']="";
                $mno="";
                $names="";
            }else{
...
<table width="90%" align="center">
    ...
    <table width="78%" align="right">
        ...
        <form method="POST"><table>
            ...
            <tr><td>Previous Amount:</td><td><input type="text" name="prevadv" value="<?php echo $prevAdvance ?>" disabled></td></tr>
            <tr><td>New Amount:</td><td><input type="text" name="amount"></td></tr>
            <tr><td>Invoice No:</td><td><input type="text" name="invoice"></td></tr>
            <tr><td></td><td><input type="submit" name="submit" value="Submit Clinical"</td></tr>
    </table></form>
        <?php echo $form ?>
        </td></tr>
    </table>
