...
if ( $_POST['search'] ) {
    ...
    $month = $smonth.' '.$syear;
    if ( empty( $stext ) ) {
        $storeqry = mysqli_query( $config, "SELECT * FROM advance WHERE advancemonth='$month'" );
    } else {
        $storeqry = mysqli_query( $config, "SELECT * FROM advance WHERE mno='$stext' and advancemonth='$month'" );
    }
}
?>
...
<?php
echo '<a href = "downloadadvance.php?smonth='.$_POST['smonth'].'&syear='.$_POST['syear'].'"><button id="smallbutton">Download</button>'
?>
</td>
